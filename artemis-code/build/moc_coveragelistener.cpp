/****************************************************************************
** Meta object code from reading C++ file 'coveragelistener.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../src/model/coverage/coveragelistener.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'coveragelistener.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_artemis__CoverageListener[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      45,   27,   26,   26, 0x0a,
     152,   88,   26,   26, 0x0a,
     240,  214,   26,   26, 0x0a,
     321,  303,   26,   26, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_artemis__CoverageListener[] = {
    "artemis::CoverageListener\0\0sourceCode,source\0"
    "slJavascriptScriptParsed(QString,QSource*)\0"
    "functionName,bytecodeSize,functionStartLine,sourceOffset,source\0"
    "slJavascriptFunctionCalled(QString,size_t,uint,uint,QSource*)\0"
    "binfo,sourceOffset,source\0"
    "slJavascriptBytecodeExecuted(ByteCodeInfoStruct,uint,QSource*)\0"
    "linenumber,source\0"
    "slJavascriptStatementExecuted(uint,QSource*)\0"
};

void artemis::CoverageListener::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        CoverageListener *_t = static_cast<CoverageListener *>(_o);
        switch (_id) {
        case 0: _t->slJavascriptScriptParsed((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QSource*(*)>(_a[2]))); break;
        case 1: _t->slJavascriptFunctionCalled((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< size_t(*)>(_a[2])),(*reinterpret_cast< uint(*)>(_a[3])),(*reinterpret_cast< uint(*)>(_a[4])),(*reinterpret_cast< QSource*(*)>(_a[5]))); break;
        case 2: _t->slJavascriptBytecodeExecuted((*reinterpret_cast< const ByteCodeInfoStruct(*)>(_a[1])),(*reinterpret_cast< uint(*)>(_a[2])),(*reinterpret_cast< QSource*(*)>(_a[3]))); break;
        case 3: _t->slJavascriptStatementExecuted((*reinterpret_cast< uint(*)>(_a[1])),(*reinterpret_cast< QSource*(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData artemis::CoverageListener::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject artemis::CoverageListener::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_artemis__CoverageListener,
      qt_meta_data_artemis__CoverageListener, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &artemis::CoverageListener::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *artemis::CoverageListener::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *artemis::CoverageListener::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_artemis__CoverageListener))
        return static_cast<void*>(const_cast< CoverageListener*>(this));
    return QObject::qt_metacast(_clname);
}

int artemis::CoverageListener::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
