/****************************************************************************
** Meta object code from reading C++ file 'artemiswebpage.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../src/runtime/browser/artemiswebpage.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'artemiswebpage.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_artemis__ArtemisWebPage[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: signature, parameters, type, tag, flags
      35,   25,   24,   24, 0x05,
      78,   74,   24,   24, 0x05,
     134,  115,   24,   24, 0x05,

       0        // eod
};

static const char qt_meta_stringdata_artemis__ArtemisWebPage[] = {
    "artemis::ArtemisWebPage\0\0frame,msg\0"
    "sigJavascriptAlert(QWebFrame*,QString)\0"
    "msg\0sigJavascriptConsoleMessage(QString)\0"
    "frame,request,type\0"
    "sigNavigationRequest(QWebFrame*,QNetworkRequest,QWebPage::NavigationTy"
    "pe)\0"
};

void artemis::ArtemisWebPage::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ArtemisWebPage *_t = static_cast<ArtemisWebPage *>(_o);
        switch (_id) {
        case 0: _t->sigJavascriptAlert((*reinterpret_cast< QWebFrame*(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        case 1: _t->sigJavascriptConsoleMessage((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 2: _t->sigNavigationRequest((*reinterpret_cast< QWebFrame*(*)>(_a[1])),(*reinterpret_cast< const QNetworkRequest(*)>(_a[2])),(*reinterpret_cast< QWebPage::NavigationType(*)>(_a[3]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData artemis::ArtemisWebPage::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject artemis::ArtemisWebPage::staticMetaObject = {
    { &QWebPage::staticMetaObject, qt_meta_stringdata_artemis__ArtemisWebPage,
      qt_meta_data_artemis__ArtemisWebPage, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &artemis::ArtemisWebPage::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *artemis::ArtemisWebPage::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *artemis::ArtemisWebPage::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_artemis__ArtemisWebPage))
        return static_cast<void*>(const_cast< ArtemisWebPage*>(this));
    return QWebPage::qt_metacast(_clname);
}

int artemis::ArtemisWebPage::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWebPage::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void artemis::ArtemisWebPage::sigJavascriptAlert(QWebFrame * _t1, QString _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void artemis::ArtemisWebPage::sigJavascriptConsoleMessage(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void artemis::ArtemisWebPage::sigNavigationRequest(QWebFrame * _t1, const QNetworkRequest & _t2, QWebPage::NavigationType _t3)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)), const_cast<void*>(reinterpret_cast<const void*>(&_t3)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_END_MOC_NAMESPACE
